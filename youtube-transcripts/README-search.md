...
- `decensor_candidates(video_id, start_char, replacement, confidence)` stores staged local-STT replacement candidates. `start_char` is the inclusive character offset of the six-character `[ __ ]` marker, `confidence` is the normalized alignment score, and candidates are not applied to `videos.transcript`.
...
Stats are served from `youtube-transcripts/data/stats-cache.json`. Regenerate that cache after DB imports or segment-density changes; otherwise the first stats API request has to scan and group the full `segments` table, which can take about a minute on Hostinger:

```bash
sqlite3 -cmd ".timeout 60000" data/transcripts.sqlite3 "PRAGMA wal_checkpoint(TRUNCATE);"
/opt/homebrew/bin/php -r 'require "/Users/diego/Desktop/Ego/public_html/youtube-transcripts/lib.php"; $db = yt_db(false); yt_refresh_channel_stats($db);'
```

## Build A Production DB With Coarser Segments

Keep the canonical dev DB at full subtitle-cue resolution. Do not keep both full and condensed segment tables in the same DB; that just makes the dev DB and upload larger. For production, build a temporary DB artifact by backing up the source DB, rewriting only the copied `segments` table, and compacting the copy. This reduces storage and stats/query overhead while leaving `videos.transcript`, `channels`, `decensor_candidates`, and both FTS tables intact.

Preview the reduction without writing a DB:

```bash
cd "/Users/diego/Desktop/Read/YouTube Channel Transcripts"
python3 refresh/build_prod_db.py --dry-run --min-spacing-seconds 5
```

Build a prod copy with at least five seconds between kept segment anchors:

```bash
cd "/Users/diego/Desktop/Read/YouTube Channel Transcripts"
python3 refresh/build_prod_db.py \
  --source-db data/transcripts.sqlite3 \
  --output-db data/transcripts.prod.sqlite3 \
  --min-spacing-seconds 5 \
  --stats-cache data/stats-cache.json \
  --overwrite
```

Use `--min-spacing-seconds 0` to keep every segment, or choose any other spacing for production. The script does not mutate the source DB and does not recalculate FTS. It preserves the copied FTS5 tables and rewrites only `segments`, then optionally writes a matching `stats-cache.json`. Delete `data/transcripts.prod.sqlite3` after uploading if local storage is tight; it is a generated artifact.
...
MLX/Metal test run with explicit defaults:
...
- `--skip-successful` now means "skip videos that already have at least one row in `decensor_candidates`."
- Reruns replace existing `decensor_candidates` rows for the same processed marker unless `--keep-existing-candidates` is passed.
- The UI has a `Patches` tab backed by `api.php?action=patches`, showing timestamped YouTube links and staged replacement candidates from `decensor_candidates`.
- Per-video and aggregate summaries include `seconds_per_marker`, `seconds_per_window`, and `whisper_rtf` while keeping the per-stage timing breakdown. Multi-video runs also include a `progress` ETA on each per-video summary.

Inspect staged candidates directly:

```bash
sqlite3 -cmd ".timeout 5000" data/transcripts.sqlite3 "
  SELECT video_id, start_char, replacement, confidence
  FROM decensor_candidates
  ORDER BY confidence ASC, video_id, start_char
  LIMIT 50;
"
```

Default windowing is cue-bounded with no extra context: `--pad-seconds 0`, `--audio-mode auto`, `--word-timestamps`, `--cue-time-tolerance 0.25`, and `--max-replacement-tokens 1`. The auto audio mode streams tiny jobs, but downloads source audio once for marker-heavy videos and seeks locally for each cue. Overlapping cue windows are merged into one non-overlapping audio window up to `--max-chunk-seconds 30`, so close censored cues share work without duplicating audio. Raise `--max-replacement-tokens` only when you want to review possible multi-word censor spans.
...
7. If a channel should not use the default English subtitle selection, add `sub_langs` to its row. This value is passed directly to `yt-dlp --sub-langs`. Use this for non-English native transcripts, especially when English captions would be translations:

```json
{ "channel": "Stromae", "url": "https://www.youtube.com/@stromae", "enabled": true, "sub_langs": "^fr$" }
```
...
rsync -a --partial --inplace --progress -e "ssh -p 65002" \
  "/Users/diego/Desktop/Read/YouTube Channel Transcripts/data/transcripts.prod.sqlite3" \
...
rsync -a --partial --progress -e "ssh -p 65002" \
  "/Users/diego/Desktop/Ego/public_html/youtube-transcripts/data/stats-cache.json" \
  "u506224278@145.223.105.221:/home/u506224278/domains/diegozc.com/public_html/youtube-transcripts/data/stats-cache.json"
```
...
Upload the generated prod DB as live `transcripts.sqlite3`, plus the matching `stats-cache.json`. The local `transcripts.sqlite3-wal` should usually be `0B`, and `transcripts.sqlite3-shm` does not need to be uploaded.
